...
class RatePopup extends HTMLElement {
  ...
  attributeChangedCallback(name: string) {
    ...
    if (name === 'open' || (name === 'review-url' && this.hasAttribute('open'))) {
  ...
  private _renderIntoPortal() {
    ...
    this._root.innerHTML = `
      <style>
        ...
        button.icon { border: 1px solid #ddd; background: transparent; width: 34px; height: 34px; border-radius: 10px; cursor: pointer; }
      ...
      <div class="overlay" aria-hidden="true">
        <div class="dialog" role="dialog" aria-modal="true">
          <div class="header">
            ...
            <button class="icon" type="button" aria-label="Close">X</button>
          </div>
  ...
  private _trapFocus(e: KeyboardEvent) {
    if (!this._root) return;
    const nodes = this._root.querySelectorAll('button, [href], iframe, [tabindex]:not([tabindex="-1"])');
    const focusable = Array.from(nodes).filter((el) => !(el as HTMLButtonElement).disabled);
