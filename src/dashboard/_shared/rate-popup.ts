const DEFAULT_REVIEW_URL = 'https://www.wix.com/app-market/add-review/a1b2c3d4-e5f6-7890-abcd-ef1234567890';
...
class RatePopup extends HTMLElement {
  ...
  private _renderIntoPortal() {
    ...
    this.$dialog?.addEventListener('mousedown', (e: MouseEvent) => e.stopPropagation());
  }
  ...
  private _getFocusable() {
    ...
    return Array.from(nodes).filter((el) => !(el as HTMLButtonElement).disabled);
  }
  ...
  private _trapFocus(e: KeyboardEvent) {
    ...
    const active = this._root?.activeElement ?? document.activeElement;
