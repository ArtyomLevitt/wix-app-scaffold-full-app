...
export const GET: APIRoute = async () => {
  try {
    ...
    try {
      const elevatedList = auth.elevate(
        (appPlans as Record<string, unknown>).listAppPlansByAppId as (
          ids: string[],
      ...
      response = await (
        appPlans as Record<string, unknown> & {
          listAppPlansByAppId: (ids: string[]) => Promise<Record<string, unknown>>;
        }
    ...
    const allAppPlans = (response.appPlans as Record<string, unknown>[]) ?? [];

    const plans: AppPlan[] = allAppPlans.flatMap((appPlanGroup) => {
      const groupPlans = (appPlanGroup.plans as Record<string, unknown>[]) ?? [];
      return groupPlans.map((plan) => ({
        vendorId: (plan.vendorId as string) ?? '',
        name: (plan.name as string) ?? '',
        benefits: ((plan.benefits as string[]) ?? []).filter(
          (b) => b && b.trim() !== '',
        ),
        prices: ((plan.prices as Record<string, unknown>[]) ?? []).map((p) => {
          const billingCycle = p.billingCycle as Record<string, unknown> | undefined;
          const cycleDuration = billingCycle?.cycleDuration as
          ...
          else if (cycleType === 'WEEK') cycleType = 'WEEKLY';
          else if (cycleType === 'DAY') cycleType = 'DAILY';
          return {
            price: pickPrice(p),
