...
export default appInstances.onAppInstanceInstalled(async (event) => {
  try {
    ...
    try {
      ...
      const categories = (properties.categories ?? {}) as Record<string, unknown>;
    ...
    await supabase.from('app_installations').upsert(
      {
        ...
        site_url: (site as Record<string, unknown>)?.url ?? null,
        site_locale: (site as Record<string, unknown>)?.locale ?? null,
