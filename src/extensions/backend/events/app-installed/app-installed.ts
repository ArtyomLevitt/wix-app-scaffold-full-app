...
export default appInstances.onAppInstanceInstalled(async (event) => {
  try {
    ...
    try {
      ...
      const secondary = categories?.secondary as string[] | undefined;
      subCategory = secondary?.[0] ?? null;
    } catch (err) {
    ...
    await supabase.from('app_installations').upsert(
      {
        ...
        site_url: (site as Record<string, unknown> | undefined)?.url ?? null,
        site_locale: (site as Record<string, unknown> | undefined)?.locale ?? null,
