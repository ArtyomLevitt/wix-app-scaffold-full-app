...
// Canonical PRPL review popup — copy verbatim into src/dashboard/_shared/rate-popup.ts
// Replace 00000000-0000-4000-8000-000000000001 in DEFAULT_REVIEW_URL with your app's UUID from Wix Dev Center.
// Triggered ONCE on first successful save (gated by localStorage).

const DEFAULT_REVIEW_URL = 'https://www.wix.com/app-market/add-review/00000000-0000-4000-8000-000000000001';
...
class RatePopup extends HTMLElement {
  ...
  attributeChangedCallback(name: string) {
    ...
    if (name === 'open') {
      this._syncFromAttrsAndOpen();
    } else if (name === 'review-url' && this.hasAttribute('open')) {
      this._syncFromAttrsAndOpen();
    }
  }
  ...
  private _renderIntoPortal() {
    ...
    this._root.innerHTML = `
      <style>
        ...
        .overlay {
          ...
          pointer-events: auto;
          font-family: var(--wix-font-family, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif);
        }
    ...
    this.$dialog?.addEventListener('mousedown', (e: any) => e.stopPropagation());
  }
  ...
  private _close() {
    ...
    queueMicrotask(() => {
      (this._lastFocused as any)?.focus?.();
  ...
  private _getFocusable() {
    ...
    return Array.from(nodes).filter((el) => !(el as any).hasAttribute?.('disabled'));
  }
  ...
  private _trapFocus(e: KeyboardEvent) {
    ...
    const first = focusable[0] as any;
    const last = focusable[focusable.length - 1] as any;
    const active = (this._root as any).activeElement || document.activeElement;
...
export function openRatePopup(reviewUrl?: string) {
  ...
  let el = document.querySelector('rate-popup') as any;
  if (!el) {
    el = document.createElement('rate-popup') as any;
