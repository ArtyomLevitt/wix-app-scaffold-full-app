...
const toUrl = (asset: { src?: string } | string): string =>
    ...
    <Card>
      <Card.Content>
        <Box direction="vertical" gap="18px">
          <Text size="medium" weight="bold">{t('moreApps.title')}</Text>
          <div style={{ display: 'flex', gap: 24, alignItems: 'stretch', flexWrap: 'wrap' }}>
            {apps.map((app) => (
              ...
              >
                <img src={app.icon} alt={app.name} style={{ width: 48, height: 48, borderRadius: 10 }} />
                <Text size="small" weight="bold" style={{ textAlign: 'center' }}>{app.name}</Text>
                <div style={{ flex: 1, display: 'flex', alignItems: 'flex-start' }}>
                  <Text size="tiny" secondary style={{ textAlign: 'center' }}>{app.desc}</Text>
                </div>
                <Button size="tiny" priority="secondary" as="a" href={app.url} target="_blank" rel="noopener noreferrer">
                  {t('button.getApp')}
                </Button>
              </div>
            ))}
          ...
          <Box direction="horizontal" verticalAlign="middle">
            ...
            <Box direction="horizontal" gap="SP1" verticalAlign="middle">
              <Text size="tiny" secondary style={{ letterSpacing: '1.2px', textTransform: 'uppercase' }}>
                {t('moreApps.poweredBy')}
              </Text>
              <img src={toUrl(purpleLogoIcon)} alt="PURPLE" style={{ height: 20, display: 'block' }} />
            </Box>
          </Box>
        </Box>
      </Card.Content>
