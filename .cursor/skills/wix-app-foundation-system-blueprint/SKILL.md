...
- [ ] **Free-tier watermark says "Powered by Purple" — NEVER the app name** — The free-plan watermark rendered in the live-site widget (and mirrored in the dashboard Live Preview) MUST read **"Powered by Purple"** (the PRPL brand), NOT `Powered by {APP_NAME}` / the app's own title. The watermark is brand attribution for Purple, the app vendor — printing the app's own name ("Powered by Pricing Plans Showcase") is meaningless to the visitor and looks like a bug. Use the literal brand string `Powered by Purple` (a plain centered text line is fine for the live widget); the richer "POWERED BY" + purple-logo lockup is the dashboard "More Apps by Us" footer pattern. The watermark MUST disappear when `isPremium` (gate it behind `{!isPremium && ...}`), and the dashboard preview MUST match the widget. Canonical reference: [`examples/share-google-drive-content/element.tsx`](examples/share-google-drive-content/element.tsx) — the `watermark` div renders `Powered by PURPLE`.
