...
> **IMPORTANT — Do NOT pass `title` to `<Page.Header>`.**
> The Wix dashboard chrome already renders the page title from the extension manifest (`extensions.dashboardPage({ title: '...' })` in your `*.extension.ts`). If you also pass a `title` prop to `<Page.Header>`, the title will render **twice** in the dashboard. Use `<Page.Header>` only for `subtitle` and `actionsBar`.
